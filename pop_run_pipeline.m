...
uilist = { { 'style' 'text' 'string' 'NEMAR pipeline' 'fontweight' 'bold' } ...
           ...
           { } ...
           ...
           { 'style' 'checkbox' 'string' 'Batch mode with default parameters (unchecked: local with custom parameters above)' 'tag' 'run_batch' } ...
...
if res.run_final_folder
    fprintf(2, 'Fresh run of pipeline for official output directory using default parameters...\n')
    % res.out_select = sprintf('/expanse/projects/nemar/openneuro/processed/%s', bidsName);
    cleanrun_command = sprintf('\nds_create_and_submit_job(''%s'', ''modeval'', ''new'');\n\n', bidsName);

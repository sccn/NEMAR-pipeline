...
addpath('/home/dtyoung/NEMAR-pipeline/nemar_plugins')
addpath('/expanse/projects/nemar/eeglab');
eeglab nogui;

processed_path = '/expanse/projects/nemar/openneuro/processed';
% processed datasets
...
for i=1:numel(dsfolders)
    ...
    studyfile = fullfile(processed_path, dsnumber, sprintf('%s.study', dsnumber));
    % [STUDY ALLEEG] = pop_loadstudy(studyfile);
    % try
    %     for i=1:numel(ALLEEG)  
    %         EEG = ALLEEG(i);
    %         nemar_plugin_iclabel_hist(EEG,'eeg');
    %     end
    
        run_pipeline(dsnumber, 'preprocess', false, 'plugin_specific', {'iclabel_hist'}, 'dataqual', false, 'run_local', false, 'modeval', 'resume');
    catch ME
        fprintf('%s\n', ME.identifier);
        fprintf('%s\n', ME.message);
        fprintf('%s\n', ME.getReport());
